...
int checkboard(void)
{
	puts("BOARD: ArchiTech Hachiko Board\n");
	return 0;
}
...
//	gpio_set( 1, 5, 1);	/* LED OFF */
	gpio_set( 1, 5, 0);	/* LED ON */
...
int board_late_init(void)
{
	...
	/* XIP in QSPI, SDRAM is system RAM, rootfs is SD Card */
//setenv("b1","bootm start 0x09000000 - 0x09800000 ; bootm loados ;fdt memory 0x08000000 0x08000000");
//set bargs 'console=ttySC2,115200 console=tty0 ignore_loglevel root=/dev/mtdblock0'
//set b_boot 'run b1 ; qspi dual; set bootargs ; fdt chosen; bootm go'
//run b_boot

	/* DEBUG BOOT (XIP) */
	/* XIP in internal RAM (0x20000000), SDRAM is system RAM, ROOTFS in QSPI */
//	setenv("d1", "fdt addr 09800000 ; fdt memory 0x08000000 0x08000000");	/* 128MB external RAM */
//	setenv("d2", "qspi dual ; fdt chosen; bootx 20000000 09800000");
//	setenv("dbootargs", "console=ttySC2,115200 console=tty0 earlyprintk root=/dev/null rootflags=physaddr=0x1880000");
//	setenv("dboot", "set bootargs ${dbootargs}; run d1 d2");

	/* DEBUG BOOT (uImage) */
	/* SDRAM is system RAM, ROOTFS in QSPI */
	setenv("d1", "bootm start 0x09000000 - 0x09800000 ; bootm loados");
	setenv("dbootargs", "console=ttySC2,115200 console=tty0 earlyprintk root=/dev/null rootflags=physaddr=0x1880000");
	setenv("dboot", "run d1; set bootargs ${dbootargs}; fdt chosen; qspi dual; bootm go");
	...
	//setenv("ipaddr", "192.168.0.55"); //asdf
	//setenv("serverip", "192.168.0.1"); //asdf
	return 0;
}
...
void led_set_state(unsigned short on)
{
	if (on)	/* turn LED on */
		gpio_set(1,5,0);
	else		/* turn LED off */
...
{
	ulong machid = MACH_TYPE_HACHIKO;
...
/* Create an alias of 'set' to the real comamnd 'setenv' */
/* Note that we had to remove 'static' from the function do_env_set() 
   in file common/cmd_nvedit.c */
/* We do this only because some early BSP instructions and app notes
   used 'set' instead of 'setenv', and that gets broken when CONFIG_CMD_SETEXPR
   is defined because now set could mean setenv or setexpr */
...
/* sdram_test */
int do_sdram_test(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	ulong base;
	ulong maxsize;
	long result;
	...
	/* The function get_ram_size() gets it's results by trying to read and
	   write memory, so it is a good way to test if SDRAM is configured
	   correctly */
	result = get_ram_size((long *)base, maxsize);

	if( result == maxsize )
		printf("PASSED!\n");
	else
		printf("FAILED! Died at address 0x%lX\n",base+result);
...
static char sdram_test_help_text[] =
	"addr size\n    - Testing for SDRAM\n"
	"\t   addr: Address of SDRAM (hex)\n"
	"\t   size: Size to check (hex)";
U_BOOT_CMD(
	sdram_test,	CONFIG_SYS_MAXARGS, 1, do_sdram_test,
	"Test SDRAM", sdram_test_help_text
);
